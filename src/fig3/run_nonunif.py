...
Given Qa, find Pd given a nonuniform bandwidth sigma and simulate harm
"""
...
if __name__ == '__main__':
    ...
    sigmas[:int(N/3)] *= 0.05
    sigmas[int(N/3):int(2*N/3)] *= 0.01
    sigmas[int(2*N/3):] *= 0.001
    ...
    if args.save:
        save_dir = './fig3/data/nonunif'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        
        # fig.savefig(os.path.join(save_dir,'nonunif.png'))
