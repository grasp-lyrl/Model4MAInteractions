...
Unknown and moving Qa, estimate Qa and find Pd and simulate harm
"""
...
def simulate(w,e=100,na=int(1000),debug=False):
    ...
    for ee in range(e):
        ...
        # moving Qa' is Qa*lognormal
        # mean = np.random.uniform(0,2) 
        # std = (np.log(np.random.uniform(0,5)**2 + 1))**.5
        # Qa += np.random.lognormal(mean,std,N)
        # Qa = Qa/Qa.sum()

        ## shifting Qa by a bin
        Qa = np.hstack([Qa[1:], Qa[0]])

        ## calculate optimal if Qa known
        star = cross_react(qa=Qa, fda=fda)

        ## sample na attackers from Qa
        qa1 = np.random.multinomial(na, Qa)
        qa = np.repeat(np.arange(N), qa1)
        ## get bin indexes for agents i.e. na to N where ii are the indeces
        qas = np.tile(qa, (N,1))
        oo = np.arange(N)
        ...
        for t in range(T):
            ## calc new Pr based on qhat
            if t % (T//(T/50)) == 0:
                w = cross_react(qa=qhat,fda=fda)
            """
            ...
            if ((fat>0).sum() == 0) or t == T-1:
                if debug:
                    print('[%05d]'%t)
                    print('fat', fat)
                ## empirical harm calculation for episode
                c.append( Fm(mat).mean())                
                ## analytical harms
                acosts_star.append(analytical_cost(Qa, star['pd']))
                acosts_ego.append(analytical_cost(Qa, Pd))
                ## wass dist
                qawds.append(wasserstein_distance(Qa,qhat))
                pdwds.append(wasserstein_distance(Pd,star['pd']))
                ## save data
                Qas.append(Qa)
                Pds.append(Pd)
                qhats.append(qhat)
                Pdstars.append(star['pd'])
                break
    ...
    return dict(harm=mm,
                ...
                qhats=qhats,
                Pdstars=Pdstars,
                acosts_ego=np.asarray(acosts_ego),
                acosts_star=np.asarray(acosts_star),
                qawds=np.asarray(qawds),
                pdwds=np.asarray(pdwds),
                c=c,
...
if __name__ == '__main__':
    ...
    plt.plot(w['x'],s['Pd'],label='$P_d$');
    plt.plot(w['x'],s['qhat'],label='$\hat{Q}_a$');
    ...
    if args.save:
        save_dir = './fig6/data/moving'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        
        # fig.savefig(os.path.join(save_dir,'est_moving.png'))
        pickle.dump(s, open(os.path.join(save_dir,'sim_moving.pkl'), 'wb'))
        pickle.dump(w, open(os.path.join(save_dir,'w_moving.pkl'), 'wb'))
